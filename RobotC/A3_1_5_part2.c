...
task main() {
  while(1) {
  	// reset brightest value to 0 if the bump switch is pressed
    brightest = SensorValue(lightSensor);
    while (SensorValue(bumpSwitch) == 0) {
    	// increase brightest, if the potentiometer value is greater than its current value
      if (SensorValue(lightSensor) < brightest) {
        brightest = SensorValue(lightSensor);
      }
    }
  }
